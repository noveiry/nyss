...
export const projectDashboardSagas = () => [
  ...
  takeEvery(
    consts.GET_PROJECT_DASHBOARD_REPORT_HEALTH_RISKS.INVOKE,
    getProjectDashboardReportHealthRisks,
  ),
...
function* openProjectDashboard({ projectId }) {
  ...
  try {
    ...
    const filtersData = yield call(
      http.get,
      `/api/projectDashboard/filters?projectId=${projectId}`,
    );
...
function* getProjectDashboardReportHealthRisks({
  projectId,
  latitude,
  longitude,
}) {
